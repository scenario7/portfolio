...
const ProjectCard = ({icon, title, description, url, numbers, technologies, github}) => {
  ...
  return (
    <div className='bg-stone-700 bg-opacity-20 text-white w-full px-5 py-5 flex flex-col md:flex-row lg:flex-row md:space-x-5 lg:space-x-5 items-center space-y-4 rounded-2xl'>
        ...
        <div className='flex flex-col items-center lg:items-start md:items-start space-y-3'>
            ...
            <p className='text-stone-400 font-dgs text-lg text-center md:text-left'>{description}</p>
            <div className='flex space-x-3'>
                {technologies.map((tech, index) => (
                    <div className=''>
                        <h1 className='bg-stone-800 rounded-full text-stone-400 text-sm px-2 py-1 outline outline-[0.5px] outline-stone-700 font-light'>{tech}</h1>
                    </div>
                ))}
            </div>
